    ...
    <section className="mx-auto w-full max-w-5xl overflow-hidden pt-16">
      ...
      <div className="relative z-10 flex max-w-2xl flex-col gap-5 px-4">
        ...
        >
          ...
          <span className="text-xs">Attendance & Marks now in one dashboard</span>
        ...
        >
          {"Manage every classroom in one place"
            .split(" ")
            .map((word, i) => (
              <motion.span
                key={i}
        ...
        >
          {"EduTrack gives Pakistani schools a single dashboard to track attendance, record marks, upload assignments, and generate performance reports — for admins and teachers alike."
            .split(" ")
            .map((word, i) => (
              <motion.span
                key={i}
        ...
        >
          ...
          >
            <Link href="/login">
              <ShieldCheckIcon data-icon="inline-start" className="mr-1.5 size-4" />
              Admin Login
            </Link>
          </Button>
          <Button asChild className="rounded-full pl-4 pr-1 py-2 text-sm active:scale-[0.97] transition-[transform] duration-[160ms] ease-[var(--ease-out)] group">
            <Link href="/login">
              <span className="mr-3">Teacher Login</span>
